...
export default function QuoteDoc({
  ...
  const displayRows = rows.length ? rows : lines;
  ...
  return (
    ...
    >
        ...
        <div>
          {logo ? (
            <div className="quote-logo" style={{ marginBottom: 10 }}>
              ...
              <img
                src={logo}
                alt={biller.name ? `${biller.name} logo` : "logo"}
                style={{
                  maxWidth: 160,
                  maxHeight: 80,
                  objectFit: "contain",
          ...
          {biller.phone && (
            <div style={{ fontSize: 12, color: "#7A7970" }}>{biller.phone}</div>
          )}
          {biller.email && (
            <div style={{ fontSize: 12, color: "#7A7970" }}>{biller.email}</div>
          )}
          {biller.ird && (
            <div style={{ fontSize: 12, color: "#7A7970" }}>IRD: {biller.ird}</div>
          )}
          {biller.gst && (
            <div style={{ fontSize: 12, color: "#3D7A5E", marginTop: 3 }}>
              GST Registered
            </div>
          )}
        </div>
      ...
      >
        ...
        <strong style={{ margin: "0 3px" }}>{validUntil}</strong> from{" "}
      ...
      >
        <div>
          ...
          <div style={{ fontSize: 12, color: "#4A4940" }}>
            {[biller.address, biller.city].filter(Boolean).join(", ")}
          </div>
          {biller.ird && (
            <div style={{ fontSize: 12, color: "#4A4940" }}>IRD: {biller.ird}</div>
          )}
        </div>
        <div>
          <div
          ...
          {client.attn && (
            <div style={{ fontSize: 12, color: "#4A4940" }}>{client.attn}</div>
          )}
          <div style={{ fontSize: 12, color: "#4A4940" }}>
            {[client.address, client.city].filter(Boolean).join(", ")}
          </div>
          {client.email && (
            <div style={{ fontSize: 12, color: "#4A4940" }}>{client.email}</div>
          )}
        </div>
      </div>
            ...
            {(
              ...
              <th
                ...
                style={{
                  ...
                  textAlign: a as "left" | "center" | "right",
                  width: w,
                }}
              >
                {h}
      ...
      >
        <div>
          ...
          <p style={{ fontSize: 12, color: "#4A4940", lineHeight: 1.7 }}>
            {notes || "—"}
          </p>
        </div>
        <div>
          ...
          {biller.gst && (
            <p style={{ marginTop: 6, fontSize: 11, color: "#B0AFA8" }}>
              All amounts include GST at 15%
            </p>
          )}
        </div>
      </div>
